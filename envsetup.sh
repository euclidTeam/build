...
EOF
    ...
    local A=""
    local i
    for i in `cat $T/build/envsetup.sh $T/vendor/euclid/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      A="$A $i"
    done
    echo $A
}
...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/euclid/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/euclid/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
function lunch()
{
    ...
    if [ "$1" ]; then
        ...
        print_lunch_menu
        echo "Which would you like? [aosp_cf_x86_64_phone-ap1a-eng]"
    ...
    if [ -z "$answer" ]
    then
        selection=aosp_cf_x86_64_phone-ap1a-eng
    ...
    echo "";
    cat $(gettop)/build/make/euclid_ascii_logo;
    echo"";
}
